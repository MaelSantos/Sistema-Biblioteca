    ...
    if (txtBuscar.trim().length > 0) {
        if (cbxTipo.options[cbxTipo.selectedIndex].value == 'cliente') {
            $.ajax({
                method: 'POST',
                url: '/Logar/',
                data: { nome: txtBuscar, cpf: txtBuscar, email: txtBuscar, telefone: txtBuscar, login: txtBuscar, senha: txtBuscar },
            ...
            $.ajax({
                method: 'POST',
                url: '/Logar/',
                data: { nome: txtBuscar, cargo: txtBuscar, email: txtBuscar, login: txtBuscar, senha: txtBuscar },
                success: function (resultado) {
            ...
            $.ajax({
                method: 'POST',
                url: '/Logar/',
                data: { locacao: txtBuscar, devolucao: txtBuscar, diaria: txtBuscar, nome: txtBuscar, cpf: txtBuscar, email: txtBuscar, telefone: txtBuscar, login: txtBuscar },
                success: function (resultado) {
                    let cabecalho = '<tr>';
                    ...
                    for (let i = 0; i < resultado.length; i++) {
                        ...
                        // let ultimaLinha = tbody.$('tr:last-child');
                        // if (resultado[i]["ativo"] == 1) {
                        //     let apagar = ultimaLinha.querySelector('.deletar');
                        //     apagar.addEventListener('click', function () {
                        //         let linha = this.parentNode.parentNode;
